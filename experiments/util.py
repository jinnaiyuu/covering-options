...
def arguments():
    ...
    parser.add_argument('--init_dist', type=float, default=0.9, help="f value larger (smaller if init_around_goal is set to true) than this thereshold goes in the initiation set")
    parser.add_argument('--term_dist', type=float, default=0.1, help="f value smaller than this thereshold goes in the termination set")
    parser.add_argument('--init_around_goal', action='store_true', help="If true, f value smaller than init_dist goes in the init set.")

    parser.add_argument('--initall', type=bool, default=True, help="If true, initiate from everywhere except from termination set")
    parser.add_argument('--reverse', action='store_true', help="If true, reverse the direction of the option to go toward the initial position.")
...
# Set of functions to get the parameters for MuJoCo
def size_scale(mdp):
    return mdp.env.MAZE_SIZE_SCALING

def maze_width(mdp):
    return len(mdp.env.MAZE_STRUCTURE[0])

def maze_height(mdp):
    return len(mdp.env.MAZE_STRUCTURE)

def agent_init_x(mdp):
    maze = mdp.env.MAZE_STRUCTURE
    for y in range(maze_height(mdp)):
        for x in range(maze_width(mdp)):
            if maze[y][x] == 'r':
                return x

def agent_init_y(mdp):
    maze = mdp.env.MAZE_STRUCTURE
    for y in range(maze_height(mdp)):
        for x in range(maze_width(mdp)):
            if maze[y][x] == 'r':
...
def bounds(mdp):
    scale = size_scale(mdp)
    init_x = agent_init_x(mdp)
    init_y = agent_init_y(mdp)
    low_bound_x, low_bound_y = scale * (1.0 - init_x - 0.5), scale * (1.0 - init_y - 0.5)
    up_bound_x, up_bound_y = scale * (maze_width(mdp) - 1.0 - init_x - 0.5), scale * (maze_height(mdp) - 1.0 - init_y - 0.5)
    return up_bound_x, low_bound_x, up_bound_y, low_bound_y
